...
function IconWrap({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return <span className={`inline-flex h-6 w-6 items-center justify-center rounded-md bg-black text-white ${className}`}>{children}</span>
}
...
export default function Home() {
  // Controlled collapse state for the sidebar
  const [collapsed, setCollapsed] = React.useState(false)
  const toggle = React.useCallback(() => setCollapsed(v => !v), [])

  const sidebarWidth = collapsed ? 56 : 256 // px
  const middleBasis = 520 // px

  return (
    <SidebarProvider>
      {/* Grid so the right panel smoothly absorbs freed space */}
      <div
              ...
              {libraryItems.map((item, index) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    {"avatar" in item && (item as any).avatar && (
                      <Avatar className="w-6 h-6">
                        <AvatarFallback className="bg-purple-100 text-purple-700 text-xs">{(item as any).avatar}</AvatarFallback>
                      </Avatar>
                    )}
                    {"avatars" in item && (item as any).avatars && (
                      <div className="flex -space-x-1">
                        {(item as any).avatars.map((avatar: string, i: number) => (
                          <Avatar key={i} className="w-6 h-6 border-2 border-white">
                            <AvatarFallback className={`text-xs ${i === 0 ? "bg-yellow-100 text-yellow-700" : "bg-blue-100 text-blue-700"}`}>
                              {avatar}
                            </AvatarFallback>
                          </Avatar>
                        ))}
                        <div className="w-6 h-6 bg-gray-2 00 rounded-full flex items-center justify-center text-xs text-gray-600 border-2 border-white">
                          {(item as any).additionalCount}
                        </div>
                      </div>
                    )}
                  </div>
